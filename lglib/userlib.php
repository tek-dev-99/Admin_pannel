...
if(isset($_GET['filter_user_name']) && !empty($_GET['filter_user_name'])){
	$filter_user_name=$_GET['filter_user_name'];
	$where.="AND user_name LIKE '%".$filter_user_name."%'";
...
if(isset($_GET['filter_full_name']) && !empty($_GET['filter_full_name'])){
	$filter_full_name=$_GET['filter_full_name'];
	$where.="AND full_name  LIKE '%".$filter_full_name."%'";
...
$sql_total= "SELECT count(*) as total FROM user_form" .$where;
...
$sql = "SELECT * FROM user_form ".$where." ORDER BY ". $sort ." ". $order ." LIMIT ". $page_start .", " . $page_limit;
